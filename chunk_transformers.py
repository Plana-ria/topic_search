...
# MPSでの使用時はbfloat16を使用
torch_dtype = torch.bfloat16 if device == "mps" else torch.float32  # 半精度はサポートされていないのでbfloat16を選択
...
def chunk_determining(latest_chunk, new_chunk, ahead_blocks):
    ...
    while True:
        ...
        ans = extract_tagged_text(generated_text, "<chunk>", "</chunk>")
